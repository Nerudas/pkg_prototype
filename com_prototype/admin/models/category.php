...
class PrototypeModelCategory extends AdminModel
{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			$item->tags->getTagIds($item->id, 'com_prototype.category');
	 ...
	 * @since  1.0.0
	 */
	public function getTable($type = 'Categories', $prefix = 'PrototypeTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		$form = $this->loadForm('com_prototype.category', 'category', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form))
		{
			return false;
		}
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_prototype.category.' . (int) $id))
			|| ($id == 0 && !$user->authorise('core.edit.state', 'com_prototype')))
		{
	...
	{
		$app  = Factory::getApplication();
		$data = Factory::getApplication()->getUserState('com_prototype.edit.category.data', array());
		if (empty($data))
		{
			$data = $this->getItem();
			...
			{
				$filters = (array) $app->getUserState('com_prototype.categories.filter');
				$data->set('access',
					$app->input->getInt('access', (!empty($filters['access']) ? $filters['access'] : Factory::getConfig()->get('access')))
				);
			}
		}
		$this->preprocessData('com_prototype.category', $data);
	...
	public function save($data)
	{
		...
		{
			$msg   = Text::_('COM_PROTOTYPE_CATEGORY_SAVE_WARNING');
